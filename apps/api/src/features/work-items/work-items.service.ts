...
export class WorkItemsService {
  ...
  async create(workspaceId: string, projectKey: string, dto: CreateWorkItemDto) {
    const sequence = await this.projects.nextSequence(workspaceId, projectKey);
    return this.items.create({ ...dto, workspaceId, projectKey: projectKey.toUpperCase(), key: `${projectKey.toUpperCase()}-${sequence}`, sequence, dueDate: dto.dueDate ? new Date(dto.dueDate) : undefined });
  }
