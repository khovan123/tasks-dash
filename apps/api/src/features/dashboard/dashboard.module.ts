...
export class DashboardService {
  ...
  async overview(workspaceId: string): Promise<DashboardOverviewResponse> {
    ...
    const members: DashboardMemberResponse[] = memberDocuments.map(
      (member) => ({
        name: member.name,
        email: member.email,
    ...
    const projectCards: DashboardProjectResponse[] = await Promise.all(
      projects.map(async (project): Promise<DashboardProjectResponse> => {
        const workflow = workflows.find(
          (item) => item.projectKey === project.key,
        );
        const doneStatusIds =
          workflow?.statuses
            .filter(
              (status) => status.category === WORKFLOW_CATEGORIES.done,
            )
        ...
        return {
          ...
          members: members.filter((member) =>
            member.projectKeys.includes(project.key),
          ),
        };
      }),
    );
...
export class DashboardController {
  ...
  @Get("overview")
  overview(
    @Headers("x-workspace-id") workspaceId = "demo",
