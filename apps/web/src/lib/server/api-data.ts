...
interface ApiFailure { ok: false; problem?: { detailKey?: string; titleKey?: string } }
...
export async function apiData<T>(path: string): Promise<T> {
  ...
  if (!response.ok || !body || body.ok !== true) {
    ...
    throw new Error(failure?.problem?.detailKey ?? `API request failed with HTTP ${response.status}.`);
  }
  return body.data;
}
