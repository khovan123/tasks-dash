...
export default async function ProjectPage({
  ...
  const [project, items, workflow, workspace] = await Promise.all([
    ...
    <main className="app-page">
      ...
      <section className="hero-panel">
        ...
        <div className="project-links">
          <span>{project.repositoryFullName ?? "Chưa có GitHub repo"}</span>
          <span>
      ...
      <section className="data-card">
        <div className="section-heading">
          <div><span>LIVE DATABASE</span><h2>Work items</h2></div>
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.key}>
                    ...
                    <td>{item.summary}</td>
                    <td>{item.type}</td>
                    <td>{workflow?.statuses.find((status) => status.id === item.statusId)?.name ?? item.statusId}</td>
