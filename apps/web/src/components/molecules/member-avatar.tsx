...
import { MemberPresenceDot } from "@/components/atoms/member-presence-dot";
...
export function MemberAvatar({
  ...
  async function handleDiscordClick(event: React.MouseEvent) {
  ...
  return (
    <Popover>
      <PopoverTrigger asChild onClick={(event) => event.stopPropagation()}>
        <button
          type="button"
          className="relative inline-flex cursor-pointer transition-opacity hover:opacity-85"
        ...
        <div className="rounded-[28px] bg-linear-to-br from-card via-card to-secondary/35 p-4">
          <div className="mb-3 flex items-start gap-3">
            <div className="relative shrink-0">
              <Avatar className="size-15 shadow-sm ring-1 ring-border/60">
                <AvatarImage src={avatarUrl} alt={name} />
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 truncate text-lg font-bold text-foreground">{name}</p>
              {email ? <p className="mt-1 truncate text-sm text-muted-foreground">{email}</p> : null}
              {resolvedPresence ? (
                <span className={cn("mt-2 inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-semibold", presenceTone)}>
                  {presenceLabel}
                </span>
              ) : null}
            </div>
          ...
          <div className="grid gap-2">
            {githubLogin ? (
              ...
              >
                <div className="grid size-8 shrink-0 place-items-center rounded-xl bg-[#24292f] text-white shadow-sm dark:bg-white dark:text-[#24292f]">
                  <Github className="size-4" />
                </div>
                <div className="min-w-0">
                  <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">GitHub</p>
                  <p className="truncate font-mono text-sm text-foreground">@{githubLogin}</p>
                </div>
              </a>
            ) : null}
            ...
            {discordUsername ? (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">Discord</p>
                  <p className="truncate font-mono text-sm text-foreground">@{discordUsername}</p>
                </div>
                <span className={cn(
                  "inline-flex shrink-0 items-center gap-1 rounded-full px-2 py-1 text-[11px] font-semibold",
                  copied ? "bg-emerald-500/12 text-emerald-700 dark:text-emerald-400" : "bg-secondary text-muted-foreground",
                )}>
