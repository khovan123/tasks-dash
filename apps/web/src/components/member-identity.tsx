  ...
  return (
    <span className={cn("inline-flex min-w-0 items-center gap-2", className)}>
      ...
      <div className="flex flex-col min-w-0">
        <span className={cn("truncate font-medium text-foreground", textClassName)} title={email ? `${name} • ${email}` : name}>
          {name}
        </span>
        {email && (
          <span className="truncate text-xs text-muted-foreground mt-0.5 leading-none" title={email}>
            {email}
          </span>
        )}
          ...
          <div className="flex items-center gap-2 mt-0.5 text-[10px]">
            {githubLogin && (
              <a
                ...
                className="inline-flex items-center gap-0.5 text-muted-foreground hover:text-primary transition-colors font-mono"
            ...
            {githubLogin && discordUsername && <span className="text-muted-foreground/30">|</span>}
            {discordUsername && (
              <button
                type="button"
                onClick={handleDiscordClick}
                className={cn(
                  "inline-flex items-center gap-0.5 text-muted-foreground hover:text-primary transition-colors font-mono",
                  copied && "text-emerald-500 hover:text-emerald-600 font-semibold"
                )}
                title={copied ? "Đã copy!" : `Discord: @${discordUsername} (Click để copy)`}
              >
                <svg className="size-3 fill-current" viewBox="0 0 127.14 96.36">
